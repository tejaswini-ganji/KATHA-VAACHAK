...
const Home = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const inputData = `Title:${storyPrompt} Genre:${genre}`;

    setLoading(true);

    try {
      const response = await fetch("https://16da-34-90-190-236.ngrok-free.app/", {
        method: "POST",
        headers: {
  ...
  const stopStoryReadout = () => {
    ...
    setIsSpeaking(false); // Update the state to indicate it's not speaking anymore
  };
  ...
  return (
    <div className="page-container">
      ...
      {/* Audio Button */}
      {generatedStory.length > 0 && (
        <button onClick={isSpeaking ? stopStoryReadout : playStoryReadout} className="audio-button">
